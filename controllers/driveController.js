...
import { createAwsDrive, createAwsFile, createAwsFolder, populateFoldersRecursive } from '../utilites/drive/drive-utilities.js';
...
export const createDrive = async (req, res) => {
  try {
    ...
    const foundHome = await Home.findOne({ home_users: user_id });
    ...
    res.status(201).send({ drive: savedDrive, message: { content: 'Successfully created drive' } })
  }
  catch (error) {
    res.status(500).send({ message: { content: 'An error occurred creating your drive', info: error.message } })
  }
}
...
export const getDrive = async (req, res) => {
  try {
    ...
    res.status(200).send({ drive: uiDrive, message: { content: 'Successfully retreived drive' } })
  }
  catch (error) {
    console.log(error)
    res.status(500).send({ message: { content: 'An error occurred creating your file', info: error.message } })
  }
}
...
export const createFolder = async (req, res) => {
  try {
    ...
    res.status(200).send({ folders: folders, message: { content: 'Successfully created folder' } })
  }
  catch (error) {
    console.log(error)
    res.status(500).send({ message: { content: 'An error occurred creating your folder', info: error.message } })
  }
}
export const getFolder = async (req, res) => {
  try {
    ...
    res.status(200).send({ folder, message: { content: 'Successfully retreived folder' } })
  }
  catch (error) {
    console.log(error)
    res.status(500).send({ message: { content: 'An error occurred creating your file', info: error.message } })
