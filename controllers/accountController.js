...
import { getAccessTokenForAccount, getBaseAccounts } from '../utilites/account/account-utilites.js';
import config from '../config.js';
import Home from '../models/home/homeModel.js';

const { plaid: { client_id, client_secret } } = config;
...
export const createAccount = async (req, res) => {
    try {
        ...
        const newAcct = new Account({ user_id, home_id, account_token: account.token, account_name: account.name });
        await newAcct.save();
        ...
        res.status(500).send({ message: { content: 'An error occurred creating your account', info: error.message } })
    }
}
...
export const getAccounts = async (req, res) => {
    ...
    try {
        ...
        const uiAccounts = await Promise.all(foundAccounts.map(acct => getBaseAccounts(acct, client)));
        res.status(200).send({ accounts: uiAccounts, message: { content: 'Successfully pulled accounts' } })
    }
    catch (error) {
...
export const getAccountToken = async (req, res) => {
    try {
        ...
        const { access_token } = await getAccessTokenForAccount(client, public_token);
        res.status(200).send({ access_token, message: { content: 'Successfully created token' } });
    }
    catch (error) {
        res.status(500).send({ message: { content: 'an error occurred getting your account credentials', info: error.message } })
    }
}
