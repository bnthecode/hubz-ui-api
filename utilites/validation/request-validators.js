...
const getErrorList = (item, input) => {
    ...
    item.rules.forEach(rule => {
        ...
        else errorItems.push({ content: rule.validationMessage, info: 'Validation Error' });
    })
    return errorItems;
}


const getValidationResult = (validationItems, body) => validationItems.reduce((acc, item) => {
...
const userValidators = {
    createUserValidation: (req, res, next) => {
        const { user } = req.body;
        const validationResult = getValidationResult(validationItems.user.createUser, user);
        return validationResult.length ? res.status(500).send({ messages: validationResult }) : next()
    }
}
