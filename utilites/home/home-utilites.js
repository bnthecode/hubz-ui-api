    ...
    baseFiles.map(fileName =>
        s3bucket.upload({ ...params, Key: `${homeId}/${creator_name}/${fileName}` },
            function (err, data) {
                ...
                fs.unlinkSync(path.relative(process.cwd(), `${creator_name}-starter-file.html`));
            }))
...
const createBaseFolders = async (homeId, creator_name) => {
    ...
    fs.appendFile(`${creator_name}-starter-file.html`,
        `<a href="data:${creator_name}-starter-file.html" target="_blank">`, function (err) {
            if (err) throw err;
