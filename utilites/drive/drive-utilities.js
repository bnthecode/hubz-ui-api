...
import { awsItemToDbFile, awsItemToDbFolder } from "../../parsers/drive/drive-parsers.js";



const { aws: { iam_user_key, iam_user_secret, bucket_name } } = config;
...
export const populateFoldersRecursive = async (folders) => Promise.all(folders.map(async (folder) => {
...
export const parseDriveFolders = (contents, homeId) => {
    const heirachy = contents.map(directory => directory.Key.split('/')
        .filter(n => n !== '' && n !== homeId && n.length > 1));
    ...
    const buildFolders = (folderList, parentFolder) => folderList.map((item) => {
        const mainFolder = []
        ...
        return {
            ...
            meta: {
                ...
                url: `https://${bucket_name}.s3.amazonaws.com/${homeId}/${mainFolder.join('/')}${parent}`
    ...
    folders.forEach((fl, index) => {
        if (index > 0) {
            if (folders[index - 1].name == fl.name) {
                const foundIndex = folders.findIndex(n => n.name === folders[index - 1].name);
...
const createBaseFiles = async (homeId, creator_name) => {
    ...
    baseFiles.map(fileName =>
        s3bucket.upload({ ...params, Key: `${homeId}/${creator_name}/${fileName}` },
            function (err, data) {
                if (err) {
                    throw err;
                }
...
const createBaseFolders = async (homeId, creator_name) => {
    ...
    baseFolders.map(folderName =>
        s3bucket.upload({ ...params, Key: `${homeId}/${folderName}/` }, function (err, data) {
            if (err) {
            }
        }))

    fs.appendFile(`${creator_name}-starter-file.txt`,
        `Hello ${creator_name}, thanks for joining!">`, function (err) {
            if (err) throw err;
