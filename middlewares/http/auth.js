...
import { getAuthHeaders, buildReqUser } from '../../utilites/auth/auth-utilities.js';

const { auth: { jwt, cookie } } = config;
const { jwt_encryption_key } = jwt;


export const httpAuthMiddleware = async (req, res, next) => {
        try {
            if (req.originalUrl !== '/api/users' && req.originalUrl !== '/api/login') {
         ...
         res.status(401).send({ message: { content: 'Authorization failed', info: error.message } });
        }
}
